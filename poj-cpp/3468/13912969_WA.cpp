...
long long create(int root, int left, int right)
{
	...
	return tree[root].total =
		create(2 * root, left, middle) + create(2 * root + 1, middle + 1, right);
}
...
long long updata(int root, int left, int right, int val)
{
	...
	if (left <= tree[root].left && tree[root].right <= right)
	{
	if(tree[root].right!=tree[root].left){	tree[root*2].mark +=val; tree[root*2+1].mark+=val;
		}
	return tree[root].total += tree[root].mark * (tree[root].right - tree[root].left + 1);

	}
	return tree[root].total =
		updata(2 * root, left, right, val) + updata(2 * root + 1, left, right, val);
}
