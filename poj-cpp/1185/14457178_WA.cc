...
int main()
{
	...
	for(int i=0; i<cnt; i++) {
		if(hi[0] & per[i]) continue;
		dp[0][i][0] = count(per[i]);
	}
	for(int i=0; i<cnt; i++) {
		if(per[i] & hi[1]) continue;
		for(int j=0; j<cnt; j++) {
			if(per[j] & hi[0]) continue;
			if(per[i] & per[j]) continue;
			dp[1][i][j] = max(dp[1][i][j], dp[0][j][0] + count(per[i]));
		}
	}
	for(int i=2; i<N; i++) {
		for(int j=0; j<cnt; j++) {
			...
			for(int k=0; k<cnt; k++) {
				if(per[k] & hi[i-1]) continue;
				if(per[j] & per[k]) continue;
				for(int m=0; m<cnt; m++) {
					if(per[m] & hi[i-2]) continue;
